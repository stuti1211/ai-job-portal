...
function App() {
  ...
  return <div className="app-shell">
    ...
    {user && <ProfilePanel profile={profile} onSave={updateProfile} />}
...
function ProfilePanel({ profile, onSave }) { const [name, setName] = useState(""); const [editing, setEditing] = useState(false); useEffect(() => { if (profile) setName(profile.name); }, [profile]); if (!profile) return null; async function submit(event) { event.preventDefault(); await onSave(name); setEditing(false); } return <section className="profile-section" id="profile"><div className="profile-heading"><div><p className="eyebrow"><span className="eyebrow-dot" />Your account</p><h2>Profile</h2></div>{!editing && <button className="refresh-button" onClick={() => setEditing(true)}><Pencil size={15} /> Edit profile</button>}</div><div className="profile-card"><div className="profile-avatar">{profile.name.slice(0, 1).toUpperCase()}</div>{editing ? <form onSubmit={submit} className="profile-form"><input required value={name} onChange={(event) => setName(event.target.value)} /><div><button className="dark-button small" type="submit">Save changes <Check size={14} /></button><button className="cancel-button" type="button" onClick={() => { setName(profile.name); setEditing(false); }}>Cancel</button></div></form> : <div className="profile-details"><strong>{profile.name}</strong><span>{profile.email}</span><em>{profile.role.toLowerCase()}</em></div>}</div></section>; }
