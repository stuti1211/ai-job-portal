...
function App() {
  ...
  useEffect(() => { loadJobs(); }, []);
  useEffect(() => { if (user) { loadDashboard(); loadProfile(); } else { setDashboard(null); setApplications([]); setEmployerJobs([]); setProfile(null); } }, [user]);
  ...
  async function handleAuth(form) {
    if (authMode === "forgot" || authMode === "reset") {
      const path = authMode === "forgot" ? "/auth/forgot-password" : "/auth/reset-password";
  ...
  async function loadProfile() { try { setProfile(await request("/profile")); } catch (error) { setNotice(error.message); } }
  async function updateProfile(name) { try { const updated = await request("/profile", { method: "PUT", body: JSON.stringify({ name }) }); setProfile(updated); setUser({ ...user, name: updated.name }); localStorage.setItem("northstar-user", JSON.stringify({ ...user, name: updated.name })); setNotice("Profile updated."); } catch (error) { setNotice(error.message); throw error; } }
  async function uploadResume(file) { const token = localStorage.getItem("northstar-token"); const body = new FormData(); body.append("resume", file); const response = await fetch(`${API_URL}/profile/resume`, { method: "POST", headers: token ? { Authorization: `Bearer ${token}` } : {}, body }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Resume upload failed"); setProfile({ ...profile, resumeUrl: data.resumeUrl }); setNotice("Resume uploaded."); }
  async function refreshDashboard() { await loadDashboard(); setNotice("Dashboard refreshed."); }
  async function saveJob(form, id) {
    const payload = { ...form, salaryMin: form.salaryMin ? Number(form.salaryMin) : null, salaryMax: form.salaryMax ? Number(form.salaryMax) : null, deadline: form.deadline || null, experience: form.experience || null, skills: form.skills.split(",").map((item) => item.trim()).filter(Boolean), benefits: form.benefits.split(",").map((item) => item.trim()).filter(Boolean) };
  ...
  return <div className="app-shell">
    ...
    {user && <ProfilePanel profile={profile} onSave={updateProfile} onUploadResume={uploadResume} onLogout={logout} />}
...
function ProfilePanel({ profile, onSave, onUploadResume, onLogout }) { const [name, setName] = useState(""); const [editing, setEditing] = useState(false); const [uploading, setUploading] = useState(false); useEffect(() => { if (profile) setName(profile.name); }, [profile]); if (!profile) return null; async function submit(event) { event.preventDefault(); await onSave(name); setEditing(false); } async function upload(event) { const file = event.target.files?.[0]; if (!file) return; setUploading(true); try { await onUploadResume(file); } finally { setUploading(false); } } return <section className="profile-section" id="profile"><div className="profile-heading"><div><p className="eyebrow"><span className="eyebrow-dot" />Your account</p><h2>Profile</h2></div><div className="profile-actions">{!editing && <button className="refresh-button" onClick={() => setEditing(true)}><Pencil size={15} /> Edit profile</button>}<button className="cancel-button" type="button" onClick={onLogout}>Log out</button></div></div><div className="profile-card"><div className="profile-avatar">{profile.name.slice(0, 1).toUpperCase()}</div>{editing ? <form onSubmit={submit} className="profile-form"><input required value={name} onChange={(event) => setName(event.target.value)} /><div><button className="dark-button small" type="submit">Save changes <Check size={14} /></button><button className="cancel-button" type="button" onClick={() => { setName(profile.name); setEditing(false); }}>Cancel</button></div></form> : <div className="profile-details"><strong>{profile.name}</strong><span>{profile.email}</span><em>{profile.role.toLowerCase()}</em>{profile.role === "CANDIDATE" && <label className="resume-upload">{uploading ? "Uploading..." : profile.resumeUrl ? "Replace resume" : "Upload resume"}<input type="file" accept=".pdf,.doc,.docx,application/pdf" onChange={upload} disabled={uploading} /></label>}</div>}</div></section>; }
...
function EmployerJobManager({ jobs, onSave, onChangeStatus, onDelete }) {
  const emptyForm = { title: "", location: "", workMode: "REMOTE", employmentType: "FULL_TIME", salaryMin: "", salaryMax: "", experience: "", deadline: "", skills: "", benefits: "", description: "" };
  const [form, setForm] = useState(emptyForm);
  const [editingId, setEditingId] = useState(null);
  const [open, setOpen] = useState(false);
  const [error, setError] = useState("");
  function edit(job) { setEditingId(job.id); setForm({ title: job.title, location: job.location, workMode: job.workMode, employmentType: job.employmentType, salaryMin: job.salaryMin || "", salaryMax: job.salaryMax || "", experience: job.experience || "", deadline: job.deadline ? job.deadline.slice(0, 10) : "", skills: (job.skills || []).join(", "), benefits: (job.benefits || []).join(", "), description: job.description }); setOpen(true); }
